...
import { descriptorForDecorator, descriptorForProperty } from './descriptor_map';
...
function mergeProps(
  ...
  for (let i = 0; i < propKeys.length; i++) {
    ...
    if (isFunction) {
      let desc: ComputedProperty | undefined | true = descriptorForDecorator(value as Function);

      if (desc !== undefined) {
        // Wrap descriptor function to implement _super() if needed
        descs[key] = giveDecoratorSuper(key, value as ComputedDecorator, desc, descs);
...
export function applyMixin(obj: { [key: string]: any }, mixins: Mixin[], _hideKeys = false) {
  ...
  let hasDidDefineProperty = typeof obj.didDefineProperty === 'function';
  ...
  for (let i = 0; i < keys.length; i++) {
    ...
    if (hasDidDefineProperty === true) {
      obj.didDefineProperty!(obj, key, value);
    }
  }
